...
Phase 3 MVP（v0.2.6）已完成。接下来建议：

1. **优先修复回收站（#31）** — 数据安全底线，工程量大但优先级高。涉及 `pages.deleted_at` 软删除 + Trash 入口 + 自动清理。
2. **解决行内格式持久化（#3）** — 编辑器核心体验，涉及 `innerHTML` 存储改造和 FTS5 搜索适配，为 XL 级任务。
3. **Phase 3 AI 增强** — 若继续深挖 AI 场景，可考虑：
   - 流式输出（streaming）替代整段返回
   - 更多 AI action：摘要、改写语气、生成代码块
   - AI 辅助标题生成

**当前判断**：回收站是产品成熟度的重要标志，建议先于行内格式持久化推进，因为后者是底层重构，可能引入较多回归风险。
...
**当前版本**：v0.2.6，GitHub 已 push，tag 已打。
...
**当前建议的下一步**：
- 优先实现 **回收站（#31）** — 数据安全底线
- 或继续完善 **Phase 3 AI 功能** — 流式输出、更多 AI action
