...
| 优先级 | 任务 | 说明 | 关联文件 |
|--------|------|------|----------|
| P2 | **版本历史** (#32) | 每次保存 snapshot 到 `page_snapshots` 表，保留最近 50 个版本。 | `backend/internal/db/db.go`<br>`frontend/src/components/SettingsModal.tsx` |
| P2 | **暗亮主题** (#28) | 所有颜色硬编码为暗色，需改为 CSS 变量 + Tailwind `darkMode: 'class'`。 | 全局 |
| P2 | **空状态引导** (#24) | 首次启动展示模板选择（空白页 / 日记 / 读书笔记 / 会议记录）。 | `frontend/src/App.tsx` |
| P2 | **替换功能** (#19) | `Ctrl+H` 页面内搜索替换。 | `frontend/src/components/PageSearch.tsx` |
| P2 | **防抖保存优化** (#35) | 1.5s 全量 PUT → 仅发送变更 block 或单 block PUT。 | `frontend/src/components/Editor.tsx` |
...
P1（骨架屏 / 大纲 / AI 流式 / 撤销重做）已全部完成。接下来建议按优先级推进：

**P2 — 中期待办**：
1. **版本历史** (#32) — 数据安全底线，每次保存 snapshot 到 `page_snapshots` 表。
2. **暗亮主题** (#28) — 用户呼声高，需将硬编码颜色改为 CSS 变量 + Tailwind `darkMode: 'class'`。
3. **空状态引导** (#24) — 首次启动模板选择，降低上手门槛。
4. **替换功能** (#19) — `Ctrl+H` 页面内搜索替换。
5. **搜索历史** (#18) — localStorage 存最近 10 条，S 级难度极易做。
6. **防抖保存优化** (#35) — 1.5s 全量 PUT → 仅发送变更 block。
...
**当前版本**：v0.3.5，GitHub 已 push，tag 已打。
