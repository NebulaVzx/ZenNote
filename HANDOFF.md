...
| 优先级 | 任务 | 说明 | 关联文件 |
|--------|------|------|----------|
| **P0** | **#12 修复代码块 Enter 错位** | 在代码块中输入代码后按 Enter，光标位置不在下一行行首，而是在当前行与下一行之间。`react-simple-code-editor` 的 textarea 与 `<pre>` 行高同步可能有问题。 | `frontend/src/components/Editor.tsx` |
...
| `frontend/src/components/Editor.tsx` | 块编辑器核心，当前有两个 pending bug |
...
1. **修复 #12 代码块 Enter 错位**（用户多次提到，体验阻断）
2. **修复 #13 拖拽不流畅**（用户多次提到）
...
**必须优先处理的 Pending Bug**：
- #12 代码块 Enter 错位：`frontend/src/components/Editor.tsx`
