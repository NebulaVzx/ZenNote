...
1. **修复首次启动窗口在 backend 就绪前显示** — 根本原因是 `try_start_backend()` 仅 `thread::sleep(800ms)` 就假定后端已就绪。但首次运行 SQLite 建表 + seed + Windows 防火墙弹窗确认可能需要数秒，导致前端在侧边栏空白状态下渲染。
   - `try_start_backend()` 改为**轮询探测**：每 250ms 尝试 `TcpStream::connect("127.0.0.1:8080")`，最多等待 30 秒
   - 探测成功后 `setup()` 才返回，窗口/webview 才开始加载前端
   - 探测超时则 kill 子进程并返回 `None`
   - 保留 v0.3.1 前端兜底逻辑作为双保险
...
| P2 | **Phase 3 完善** | AI 功能可继续增强：流式输出、更多 action（摘要 / 改写语气）、AI 生成图片/表格提示。 | `frontend/src/components/Editor.tsx`<br>`backend/internal/api/ai.go` |
...
v0.3.0（Markdown 互操作 + 图片插入）已全部完成。接下来建议按优化后的优先级推进：

**P1 — 近期优先**：
1. **AI 增强** — 流式输出、更多 action（摘要 / 改写语气 / 代码生成）。产品差异化核心。
2. **Undo/Redo** (#11) — 编辑器体验天花板，当前依赖浏览器默认行为不可靠。
3. **骨架屏** (#23) — 纯 CSS 替代 "Loading..."，S 级难度，投入小见效快。

**P2 — 中期待办**：
4. **版本历史** (#32) — 数据安全底线，每次保存 snapshot 到 `page_snapshots` 表。
5. **暗亮主题** (#28) — 用户呼声高，需将硬编码颜色改为 CSS 变量。
6. **空状态引导** (#24) — 首次启动模板选择，降低上手门槛。
7. **防抖保存优化** (#35) — 1.5s 全量 PUT → 仅发送变更 block。
8. **替换功能** (#19) — `Ctrl+H` 页面内搜索替换。
9. **搜索历史** (#18) — localStorage 存最近 10 条，S 级难度极易做。
...
**当前版本**：v0.3.0，GitHub 已 push，tag 已打。
...
**当前建议的下一步**：
- 继续 **Phase 3 AI 增强** — 流式输出、更多 AI action
- 或继续 **P2 功能完善** — 骨架屏、空状态引导、版本历史、Undo/Redo
