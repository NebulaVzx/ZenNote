...
| 优先级 | 任务 | 说明 | 关联文件 |
|--------|------|------|----------|
| P1 | **自动同步定时器** | `auto_sync` 配置存在但后端无定时轮询。 | `backend/internal/sync/` |
...
v0.2.8 行内格式持久化已完成。接下来建议：

1. **P1 剩余项**：自动同步定时器（#20）、右键菜单（#15）、页面收藏（#13）、最近编辑（#14）。
2. **Phase 3 AI 增强** — 若继续深挖 AI 场景，可考虑：
   - 流式输出（streaming）替代整段返回
   - 更多 AI action：摘要、改写语气、生成代码块
   - AI 辅助标题生成

**当前判断**：P1 剩余项是体验完善的关键，建议按优先级逐个推进。
...
**当前版本**：v0.2.8，GitHub 已 push，tag 已打。
...
**当前建议的下一步**：
- 继续 **P1 剩余项** — 自动同步定时器（#20）、右键菜单（#15）、页面收藏（#13）、最近编辑（#14）
- 或继续 **Phase 3 AI 增强** — 流式输出、更多 AI action
