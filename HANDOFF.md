...
| 优先级 | 任务 | 说明 | 关联文件 |
|--------|------|------|----------|
| P0 | **行内格式持久化** | 当前 `Ctrl+B/I/S` 产生的 HTML 标签在切换 active/inactive 块时会丢失，因为 `content` 存的是 `innerText` 而非 `innerHTML`。 | `frontend/src/components/Editor.tsx` |
...
v0.2.7 P0/P1 Batch 1 已完成。接下来建议：

1. **解决行内格式持久化（#3）** — 编辑器核心体验，涉及 `innerHTML` 存储改造和 FTS5 搜索适配，为 XL 级任务，建议单独开一个迭代专攻。
2. **P1 剩余项**：自动同步定时器（#20）、右键菜单（#15）、页面收藏（#13）、最近编辑（#14）。
3. **Phase 3 AI 增强** — 若继续深挖 AI 场景，可考虑：
   - 流式输出（streaming）替代整段返回
   - 更多 AI action：摘要、改写语气、生成代码块
   - AI 辅助标题生成

**当前判断**：行内格式持久化是体验分水岭，但工程量大，建议作为下一个 dedicated batch 重点攻坚。
...
**当前版本**：v0.2.7，GitHub 已 push，tag 已打。
...
**当前建议的下一步**：
- 优先解决 **行内格式持久化（#3）** — 编辑器核心体验，建议单独开一个迭代专攻
- 或继续 **P1 剩余项** — 自动同步定时器（#20）、右键菜单（#15）、页面收藏（#13）
